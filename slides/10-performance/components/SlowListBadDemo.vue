...
export default function App() {
  ...
  // Lista nu depinde de text — dar se re-renderează la fiecare tastă!
  ...
  return (
    <div style={{
      ...
      <h3 style={styles.title}>Scrie în input și observă lag-ul!</h3>
      <input
        value={text}
...
const styles = {
  ...
  input: {
    padding: '8px 12px', fontSize: '15px', border: '2px solid #e2e8f0',
...
const listCode = `import { useRef } from 'react';
...
function ExpensiveItem({ name }) {
  // Simulăm un calcul costisitor pentru fiecare element
  let total = 0;
...
const styles = {
  ...
  header: {
    ...
    padding: '8px 0', borderBottom: '2px solid #e2e8f0', marginBottom: '8px',
  },
  badge: {
