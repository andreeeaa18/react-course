...
// Simulăm o funcție costisitoare de filtrare
function filterAndSort(items, query) {
  console.time('filterAndSort');
  // Simulăm delay — în realitate ar fi un dataset imens
  let result = [...items];
  for (let i = 0; i < 200; i++) {
...
export default function App() {
  ...
  // BUG: se recalculează la FIECARE render — inclusiv la toggle dark mode!
  ...
  return (
    <div style={{
      ...
      <p style={styles.hint}>
        Apasă "Toggle tema" și observă delay-ul! (vezi Console)
      </p>
...
const styles = {
  ...
  input: {
    padding: '8px 12px', fontSize: '14px', border: '2px solid #e2e8f0',
  ...
  hint: { color: '#ef4444', fontSize: '13px', fontWeight: 'bold', margin: '4px 0' },
