...
const code = `import { useReducer, useState } from 'react';
    ...
    <div style={styles.container}>
      ...
      <div style={styles.inputRow}>
        <input
          ...
          style={styles.input}
        />
        <button onClick={handleAdd} style={styles.addBtn}>Adaugă</button>
      </div>
      <ul style={styles.list}>
        {tasks.map(task => (
          <li key={task.id} style={styles.item}>
            ...
            <button onClick={() => dispatch({ type: 'deleted', id: task.id })} style={styles.deleteBtn}>✕</button>
          </li>
        ))}
      </ul>
      <p style={styles.counter}>{tasks.filter(t => !t.done).length} task-uri rămase</p>
    </div>
  );
}
...
const styles = {
  ...
  title: { fontSize: '18px', fontWeight: 'bold', marginBottom: '14px', color: '#059669' },
  ...
  addBtn: { padding: '8px 16px', background: '#059669', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' },
  list: { listStyle: 'none', padding: 0, margin: 0 },
  item: { display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', marginBottom: '8px', background: '#f0fdf4', borderRadius: '8px', border: '1px solid #bbf7d0' },
  checkbox: { width: '16px', height: '16px', cursor: 'pointer', accentColor: '#059669' },
